...
from adapters.key_vault_auth import (
    get_key_vault_session,
    get_sharepoint_credentials,
)
...
from application.process_weekly_metrics_remote import (
    process_metric_remote,
)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--file-name",
        required=True,
        help="Weekly file name to process",
    )

    parser.add_argument(
        "--client-secret",
        required=True,
        help="Key Vault client secret",
    )
...
) -> tuple[str, GraphCredentials]:

    session = get_key_vault_session(
        vault_client_secret
    )
...
def main() -> None:
    ...
    try:
        ...
        input_files = list_input_files_graph(
            context,
            f"{BASE_FOLDER}/Input",
        )
        ...
        matching = [
            ...
            if file["name"] == args.file_name
        ]

        if not matching:
            raise ATError(
                "ERR001",
                f"File {args.file_name} "
                "not found in Input",
            )

        item_id = matching[0]["id"]

        weekly_bytes = download_file_graph(
            context,
            item_id,
        )

        week_data = read_weekly_excel(
            weekly_bytes,
        ...
        master_bytes = get_file_by_path(
            context,
            f"{BASE_FOLDER}/Parametric Files/"
            "AT_Employees.xlsx",
        )
        ...
        if master_bytes is None:
            raise ATError(
                "ERR025",
                "File AT_Internal_Metrics/"
                "Parametric Files/"
                "AT_Employees.xlsx not found",
            )

        master_employees = read_excel(
            master_bytes
        )

        week_data = process_metric_remote(
            context,
            week_data,
            master_employees,
        )
        ...
        start_row = get_next_block_start_row(
            worksheet
        )
